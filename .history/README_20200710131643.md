...
1. What is the DOM?
  ...
  The DOM is the Document Object Model. It is a virtual tree of objects and nodes so that programming languages may edit and display features of the DOM to the browser. The DOM brings together the HTML source and the browser windown objects so that a webpage may be displayed correctly.

2. What is an event?

  An event is when some type of action is take on a DOM node or object.

3. What is an event listener?

  An event listener waits for a specific event to take place on a specific object and performs a callback function when that event takes place.

4. Why would we convert a NodeList into an Array?

  So that we can run the same code over many different nodes at once with methods like `.forEach` and `.map`.

5. What is a component?

  Components are reusable bits of code that we right to create DOM nodes and objects.
